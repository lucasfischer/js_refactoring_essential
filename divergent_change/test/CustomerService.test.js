...
describe("CustomerService", () => {
    ...
    describe("determineAccountStatus", () => {
        it("should return INACTIVE when days over 365", () => {
            assert.equal(
                service.determineAccountStatus(366),
                "INACTIVE"
            );
        });
        ...
        it("should return DORMANT when between 31 and 365", () => {
            assert.equal(
                service.determineAccountStatus(100),
                "DORMANT"
            );
        });
        ...
        it("should return ACTIVE when 30 days or less", () => {
            assert.equal(
                service.determineAccountStatus(30),
                "ACTIVE"
            );
            assert.equal(
                service.determineAccountStatus(0),
                "ACTIVE"
            );
        });
        ...
        it("should treat negative days as active", () => {
            assert.equal(
                service.determineAccountStatus(-10),
                "ACTIVE"
            );
        });
    });
